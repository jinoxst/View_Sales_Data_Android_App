...
package jp.ne.jinoxst.mas.itg.uriage.lib;
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public interface IconSwitchTabProvider {
        public int getPageOnIconResId(int posistion);
    ...
    private int[] onIconArr;
    private int[] offIconArr;
    ...
    public void setViewPager(ViewPager pager,  ICallback cb) {
        this.cb = cb;
        this.pager = pager;
    ...
    public void notifyDataSetChanged() {
        ...
        onIconArr = new int[tabCount];
        offIconArr = new int[tabCount];

        for (int i = 0; i < tabCount; i++) {

            if (pager.getAdapter() instanceof IconTabProvider) {
                addIconTab(i, ((IconTabProvider) pager.getAdapter()).getPageIconResId(i));
            } else if(pager.getAdapter() instanceof IconSwitchTabProvider){
                addSwitchIconTab(i, ((IconSwitchTabProvider) pager.getAdapter()).getPageOffIconResId(i));
                offIconArr[i] = ((IconSwitchTabProvider) pager.getAdapter()).getPageOffIconResId(i);
                onIconArr[i] = ((IconSwitchTabProvider) pager.getAdapter()).getPageOnIconResId(i);
            } else{
    ...
    private void initTabIcon(final int position) {
        this.cb.pageSelected(position);
        ImageButton tab = (ImageButton)tabsContainer.getChildAt(position);
        tab.setFocusable(true);
        tab.setImageResource(onIconArr[position]);
    ...
    private void changeTabIcon(final int position) {
        this.cb.pageSelected(position);
        ImageButton tab = (ImageButton)tabsContainer.getChildAt(position);
        tab.setFocusable(true);
        tab.setImageResource(onIconArr[position]);
        ...
        for(int i=0;i<tabCount;i++){
            if(i != position){
                ImageButton icon = (ImageButton)tabsContainer.getChildAt(i);
                icon.setFocusable(true);
                icon.setImageResource(offIconArr[i]);
                final int j = i;
    ...
    private void addSwitchIconTab(final int position, int resId) {
        ImageButton tab = new ImageButton(getContext());
        tab.setFocusable(true);
        tab.setImageResource(resId);
